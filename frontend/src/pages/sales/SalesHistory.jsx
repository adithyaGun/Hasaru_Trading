...
const SalesHistory = () => {
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      const pos = salesData.filter(s => s.channel === 'pos').reduce((sum, sale) => sum + parseFloat(sale.total_amount), 0);
      const online = salesData.filter(s => s.channel === 'online').reduce((sum, sale) => sum + parseFloat(sale.total_amount), 0);
          ...
          <>
            <Descriptions bordered column={2} size="small">
              <Descriptions.Item label="Channel">
                <Tag color={selectedSale.channel === 'pos' ? 'blue' : 'green'}>
                  {selectedSale.channel?.toUpperCase()}
                </Tag>
              </Descriptions.Item>
